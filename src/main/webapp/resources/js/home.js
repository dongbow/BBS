$(function(){
	$(document).ready(function(){
		var hg=$('#rhomecont').height();
		$('#homeleftmenu').css('height',hg);
	});
...
function autoPPSel(id,val){
	var sel_len=$('#'+id).length;
	for(var i=0;i<sel_len;i++){
		if ($('#'+id).options[i].val()==val){
...
$(document).ready(function(){
	var vall = $('#nowemail').text();
	val = vall.split('@')[0];
	bf = vall.split('@')[1];
	d=val.substring(0,2);
	g=val.substring(val.length-1,val.length);
	s='*****';
	var r='';
	r=d+s+g+'@'+bf;
	$('#nowemail').text(r);
